import { readUint16, readUint32, readUtf8 } from '../binary-readers'
...
const parse = function(data, offset, length)
{
  ...
  var tableVersion = readUint16(data, offset);  offset += 2;
  var svgDocIndexOffset = readUint32(data, offset);  offset += 4;
  var reserved = readUint32(data, offset); offset += 4;

  offset = svgDocIndexOffset + offset0;

  var numEntries = readUint16(data, offset);  offset += 2;

  for(var i=0; i<numEntries; i++)
  {
    var startGlyphID = readUint16(data, offset);  offset += 2;
    var endGlyphID   = readUint16(data, offset);  offset += 2;
    var svgDocOffset = readUint32  (data, offset);  offset += 4;
    var svgDocLength = readUint32  (data, offset);  offset += 4;

    var sbuf = new Uint8Array(data.buffer, offset0 + svgDocOffset + svgDocIndexOffset, svgDocLength);
    var svg = readUtf8(sbuf, 0, sbuf.length);
...
{
  var pth: any = {cmds:[], crds:[]};
  if(str==null) return pth;

  var prsr = new DOMParser();
  var doc = prsr["parseFromString"](str,"image/svg+xml");

  var svg = doc.firstChild as Element

  while(svg.tagName!="svg") svg = svg.nextSibling as Element

  var vb: any = svg.getAttribute("viewBox")!
  if(vb) vb = vb.trim().split(" ").map(parseFloat);  else   vb = [0,0,1000,1000];

  _toPath(svg.children, pth);

  for(var i=0; i<pth.crds.length; i+=2) {
    var x: any = pth.crds[i]
    var y: any = pth.crds[i+1];
    x -= vb[0];
    y -= vb[1];
    y = -y;
    pth.crds[i] = x;
...
const _toPath = function(nds, pth, fill?) {
  for(var ni=0; ni<nds.length; ni++) {
    ...
    if(tn=="g") _toPath(nd.children, pth, cfl);
      ...
      _toksToPath(toks, pth);  pth.cmds.push("X");
    }
    else if(tn=="defs") {}
