...
import { sequenceReader, SequenceReader } from '../binary-readers/sequence-reader';
import { createSequence } from '../util'

const parse = (data: Uint8Array ) => {
  const reader = sequenceReader( data, 0 )
  ...
  const names = createSequence(
    nameIndices.length - 1,
    i =>
      readAscii(
        data,
        offset + nameIndices[ i ],
        nameIndices[ i + 1 ] - nameIndices[ i ]
      )
  )
...
const readIndices = ( reader: SequenceReader ) => {
  ...
  if ( offsetSize === 1 ){
    ...
    for ( var i = 0; i < count; i++ ){
      indices.push( readUint32( data, offset + i * 3 - 1 ) & 0x00ffffff )
    }
...
const readIndices2 = (data: Uint8Array, offset: number, indices: number[] ) => {
  //const indices: number[] = []
